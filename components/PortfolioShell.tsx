...
export function PortfolioShell() {
  ...
  const reveal = (direction: -1 | 1 = -1, delay = 0): MotionProps =>
    reduced
      ? {}
      : {
          ...baseMp,
  ...
  return (
    <main className="min-h-screen">
      ...
      {/* ══════════════════════════ HERO (dark) ══════════════════════════ */}
      <section id="home" className="hero-surface relative min-h-screen scroll-mt-24 overflow-hidden text-white">
        <div className="container relative grid min-h-screen items-center gap-12 py-32 lg:grid-cols-[1.15fr_0.85fr]">
          ...
          <motion.div {...reveal(-1)}>
            <h1 className="mt-4 text-5xl font-bold leading-[1.08] tracking-tight sm:text-6xl lg:text-7xl">
              Hi, I&apos;m <br/>
              <span className="hero-name">Subhav Kumar</span>
            </h1>
          ...
          <motion.div {...reveal(1, 0.12)} className="flex flex-col items-center gap-6">
      ...
      <Section id="about" label="About Me" title="A bit about myself" bg="bg-cream">
        <div className="grid items-start gap-10 lg:grid-cols-[1fr_1fr]">
          ...
          <motion.div {...reveal(1, 0.08)} className="grid grid-cols-2 gap-4">
      ...
      <Section id="research" label="Research" title="Published and ongoing work" bg="bg-sand">
        <div className="grid gap-5 lg:grid-cols-2">
          {research.map((item, i) => (
            <motion.article key={item.title} {...reveal(i % 2 === 0 ? -1 : 1, i * 0.06)} className="card">
      ...
      <Section id="experience" label="Experience" title="Where I've worked" bg="bg-cream">
        <div className="space-y-5">
          {experience.map((item, i) => (
            <motion.article key={item.role} {...reveal(i % 2 === 0 ? -1 : 1, i * 0.06)} className="card grid gap-4 lg:grid-cols-[0.3fr_0.7fr]">
              <div>
      ...
      <Section id="projects" label="Projects" title="Things I've built" bg="bg-sand">
        ...
        <div className="grid gap-5 lg:grid-cols-2">
          {filteredProjects.map((project, i) => (
            <motion.article key={project.title} {...reveal(i % 2 === 0 ? -1 : 1, i * 0.04)} className="card">
      ...
      <Section id="skills" label="Skills" title="My technical toolkit" bg="bg-cream">
        <div className="grid gap-5 md:grid-cols-2">
          {skillGroups.map((group, i) => (
            <motion.div key={group.title} {...reveal(i % 2 === 0 ? -1 : 1, i * 0.05)} className="card">
      ...
      <Section id="certifications" label="Certifications" title="Verified credentials" bg="bg-sand">
        <div className="grid gap-3 lg:grid-cols-2">
          {certifications.map((item, i) => (
            <motion.a key={item.title} {...reveal(i % 2 === 0 ? -1 : 1, i * 0.04)} href={item.href} target="_blank" rel="noreferrer" className="resume-link">
      ...
      <Section id="achievements" label="Achievements" title="Recognition and milestones" bg="bg-cream">
        <motion.div {...reveal(-1)} className="card">
      ...
      <Section id="resume" label="Resume" title="View my full resume" bg="bg-sand">
        <motion.div {...reveal(1)} className="flex flex-col gap-4 sm:flex-row sm:items-center">
          <a className="btn-outline justify-center" href="/resume">
            <FileText className="h-4 w-4" /> Web Resume
          </a>
          <a className="btn-primary justify-center" href="/resume_subhav.pdf" target="_blank" rel="noreferrer">
            Download PDF
          </a>
        </motion.div>
      </Section>
      ...
      {/* ══════════════════════════ CONTACT (dark) ══════════════════════════ */}
      <section id="contact" className="hero-surface scroll-mt-24 py-20 text-white sm:py-24">
        <div className="container grid gap-12 lg:grid-cols-[0.9fr_1.1fr]">
          ...
          <motion.form {...reveal(1, 0.06)} onSubmit={handleSubmit} className="contact-form">
            <label>
              <span>Name</span>
      ...
      {menuOpen ? (
        <div className="mobile-panel">
          <div className="grid gap-1">
            {sections.map((s) => (
              <a key={s.id} href={s.href} onClick={() => setMenuOpen(false)} className="mobile-nav-link">{s.label}</a>
            ))}
          </div>
        </div>
      ) : null}
